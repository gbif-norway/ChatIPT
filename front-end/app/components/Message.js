...
const Message = ({ message, toolResult = null }) => {
    // if (!message.openai_obj.content) { return null; }
    if (message.role == 'system') { return null; }
    var python_calls = [];
    if (message.role === 'assistant' && message.openai_obj.tool_calls) {
        // Problem is it may call python more than once? 
        // Then we have a single message with all the calls, and bunch of messages after with the results
        // I guess it's not going to happen so often so let's leave it as is for now 
